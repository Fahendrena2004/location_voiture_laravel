<x-layouts::app :title="__('Nouvelle Location')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl max-w-2xl mx-auto">
        ...
        <form action="{{ route('locations.store') }}" method="POST" class="space-y-4" id="location-form"
            x-data="{ 
                avecChauffeur: {{ old('avec_chauffeur', 0) ? 'true' : 'false' }}, 
                statut: '{{ old('statut', 'en cours') }}',
                updateTotal() {
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <flux:input type="date" name="date_debut" @change="updateTotal()" label="Date de début" value="{{ old('date_debut', date('Y-m-d')) }}" required />
                <div>
                    <flux:input type="date" name="date_fin" @change="updateTotal()" label="Date de fin" value="{{ old('date_fin', date('Y-m-d', strtotime('+1 day'))) }}" required />
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="col-span-1 md:col-span-2" x-show="statut === 'terminée'" x-cloak>
                    <flux:input type="date" name="date_retour" id="date_retour_input" @change="updateTotal()" label="Date de retour réelle" value="{{ old('date_retour') }}" />
