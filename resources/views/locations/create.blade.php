<x-layouts::app :title="__('Nouvelle Location')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl max-w-2xl mx-auto">
        ...
        <form action="{{ route('locations.store') }}" method="POST" class="space-y-6" id="location-form">
            @csrf
            ...
            <flux:field>
                ...
                <select name="voiture_id" required onchange="updateTotal()" class="w-full bg-white dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 rounded-lg p-2 text-sm">
                    <option value="" disabled {{ !old('voiture_id') ? 'selected' : '' }}>Choisir une voiture...</option>
                    @foreach($voitures as $voiture)
                        <option value="{{ $voiture->id }}" data-prix="{{ $voiture->prix_journalier }}" {{ old('voiture_id') == $voiture->id ? 'selected' : '' }}>
            ...
            <div class="space-y-4">
                <flux:field>
                    ...
                    <flux:switch name="avec_chauffeur" id="avec_chauffeur_switch" label="Louer avec chauffeur ?" value="1" onchange="toggleChauffeur()" {{ old('avec_chauffeur') ? 'checked' : '' }} />
                </flux:field>

                <div id="chauffeur-selection" style="display: {{ old('avec_chauffeur') ? 'block' : 'none' }}">
                    <flux:field>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <flux:input type="date" name="date_debut" onchange="updateTotal()" label="Date de début" value="{{ old('date_debut', date('Y-m-d')) }}" required />
                <flux:input type="date" name="date_fin" onchange="updateTotal()" label="Date de fin" value="{{ old('date_fin', date('Y-m-d', strtotime('+1 day'))) }}" required />
            </div>
            @error('date_debut') <flux:error>{{ $message }}</flux:error> @enderror
            @error('date_fin') <flux:error>{{ $message }}</flux:error> @enderror

            <flux:input type="number" step="0.01" name="tarif_total" id="tarif_total_input" value="{{ old('tarif_total') }}" label="Tarif Total (€)" required />
