<x-layouts::app :title="__('Nouvelle Location')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl max-w-2xl mx-auto">
        ...
        <form action="{{ route('locations.store') }}" method="POST" class="space-y-6">
            ...
            <flux:select name="voiture_id" id="voiture_select" label="Voiture" required>
                <flux:select.option value="" disabled selected>Choisir une voiture...</flux:select.option>
                @foreach($voitures as $voiture)
                    <flux:select.option value="{{ $voiture->id }}" data-prix="{{ $voiture->prix_journalier }}" {{ old('voiture_id') == $voiture->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <flux:input type="date" name="date_debut" id="date_debut" label="Date de début" value="{{ old('date_debut', date('Y-m-d')) }}" required />
                <flux:input type="date" name="date_fin" id="date_fin" label="Date de fin" value="{{ old('date_fin', date('Y-m-d', strtotime('+1 day'))) }}" required />
            </div>

            <flux:input type="number" step="0.01" name="tarif_total" id="tarif_total" label="Tarif Total (€)" value="{{ old('tarif_total') }}" required />

            <flux:select name="statut" label="Statut" required>
                <flux:select.option value="en cours" {{ old('statut') === 'en cours' ? 'selected' : '' }}>En cours</flux:select.option>
                <flux:select.option value="terminée" {{ old('statut') === 'terminée' ? 'selected' : '' }}>Terminée</flux:select.option>
