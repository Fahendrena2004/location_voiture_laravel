<x-layouts::app :title="__('Clients')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            ...
            <flux:button href="{{ route('clients.create') }}" wire:navigate icon="plus" variant="primary">Nouveau Client
            </flux:button>
        </div>
                        ...
                        <flux:table.row>
                            <flux:table.cell>
                                <div class="flex justify-start">
                                    @if($client->type === 'personne')
                                        <flux:badge icon="user" variant="ghost" size="sm" class="text-blue-600">Pers.
                                        </flux:badge>
                                    @else
                                        <flux:badge icon="building-office" variant="ghost" size="sm" class="text-emerald-600">
                                            Assoc.</flux:badge>
                                    @endif
                                </div>
                            </flux:table.cell>
                            <flux:table.cell class="font-medium text-zinc-900 dark:text-white">
                                <div class="truncate" style="max-width: 250px;"
                                    title="{{ $client->type === 'association' ? $client->raison_sociale : $client->nom }}">
                                    {{ $client->type === 'association' ? $client->raison_sociale : $client->nom }}
                                </div>
                            </flux:table.cell>
                            <flux:table.cell>
                                <div class="truncate" style="max-width: 150px;"
                                    title="{{ $client->type === 'personne' ? ($client->prenom ?? '-') : '-' }}">
                            ...
                            <flux:table.cell>
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item href="{{ route('clients.show', $client) }}" wire:navigate
                                            icon="eye">Voir</flux:menu.item>
                                        <flux:menu.item href="{{ route('clients.edit', $client) }}" wire:navigate
