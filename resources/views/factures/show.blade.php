            ...
            <div class="flex items-center gap-3">
                <form method="GET" action="{{ route('factures.show', $facture) }}" class="flex items-center gap-2">
                    ...
                    <select name="template" id="template" onchange="this.form.submit()"
                        ...
                        <option value="classic" {{ request('template', 'classic') === 'classic' ? 'selected' : '' }}>Classique</option>
                ...
                <flux:button as="a" href="{{ route('factures.show', ['facture' => $facture->id, 'template' => request('template', 'classic'), 'print' => 1]) }}" target="_blank" icon="printer" variant="primary">Imprimer</flux:button>
            </div>
        </div>
