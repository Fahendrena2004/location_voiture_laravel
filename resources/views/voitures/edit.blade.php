<x-layouts::app :title="__('Modifier Voiture')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl max-w-2xl mx-auto">
        ...
        <form action="{{ route('voitures.update', $voiture) }}" method="POST" class="space-y-6">
            ...
            <flux:input name="immatriculation" label="Immatriculation" :value="old('immatriculation', $voiture->immatriculation)" required />

            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <flux:input name="couleur" label="Couleur" :value="old('couleur', $voiture->couleur)" />
                <flux:input type="number" step="0.01" name="prix_journalier" label="Prix journalier (€)" :value="old('prix_journalier', $voiture->prix_journalier)" required />
            </div>

            <flux:select name="statut" label="Statut" required>
                <flux:select.option value="disponible" {{ old('statut', $voiture->statut) === 'disponible' ? 'selected' : '' }}>Disponible</flux:select.option>
                <flux:select.option value="louée" {{ old('statut', $voiture->statut) === 'louée' ? 'selected' : '' }}>Louée</flux:select.option>
                <flux:select.option value="en entretien" {{ old('statut', $voiture->statut) === 'en entretien' ? 'selected' : '' }}>En entretien</flux:select.option>
            </flux:select>
