<x-layouts::app :title="__('Entretiens')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        <div class="flex items-center justify-between">
            ...
            <flux:button href="{{ route('entretiens.create') }}" wire:navigate icon="plus" variant="primary">Nouvel
                Entretien</flux:button>
        </div>
            ...
            <flux:table>
                ...
                <flux:table.rows>
                    @foreach($entretiens as $entretien)
                        <flux:table.row>
                            <flux:table.cell>{{ $entretien->voiture->marque }} {{ $entretien->voiture->modele }}
                                ({{ $entretien->voiture->immatriculation }})</flux:table.cell>
                            <flux:table.cell>{{ \Carbon\Carbon::parse($entretien->date_entretien)->format('d/m/Y') }}
                            </flux:table.cell>
                            <flux:table.cell>
                            ...
                            <flux:table.cell>
                                <flux:dropdown>
                                    ...
                                    <flux:menu>
                                        <flux:menu.item href="{{ route('entretiens.show', $entretien) }}" wire:navigate
                                            icon="eye">Voir</flux:menu.item>
                                        <flux:menu.item href="{{ route('entretiens.edit', $entretien) }}" wire:navigate
