        ...
        <form action="{{ route('entretiens.update', $entretien) }}" method="POST" class="space-y-6">
            ...
            <flux:select name="voiture_id" label="Voiture" required>
                @foreach($voitures as $voiture)
                    <flux:select.option value="{{ $voiture->id }}" {{ old('voiture_id', $entretien->voiture_id) == $voiture->id ? 'selected' : '' }}>
                        {{ $voiture->marque }} {{ $voiture->modele }} ({{ $voiture->immatriculation }})
                    </flux:select.option>
                @endforeach
            </flux:select>

            <flux:input type="date" name="date_entretien" label="Date de l'entretien" value="{{ old('date_entretien', $entretien->date_entretien) }}" required />

            <flux:textarea name="description" label="Description des travaux" required>{{ old('description', $entretien->description) }}</flux:textarea>

            <flux:input type="number" step="0.01" name="cout" label="Coût de l'entretien (€)" value="{{ old('cout', $entretien->cout) }}" required />
