<x-layouts::app :title="__('Modifier Compte de Paiement')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl max-w-2xl mx-auto">
        <div class="flex items-center gap-4">
            ...
            <flux:heading size="xl">Modifier Compte de Paiement</flux:heading>
        </div>

        <form action="{{ route('comptes.update', $compte) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <flux:field>
                ...
                <select name="type" required
                    ...
                    <option value="bancaire" {{ old('type', $compte->type) === 'bancaire' ? 'selected' : '' }}>Banque
                    </option>
                    <option value="mobile_money" {{ old('type', $compte->type) === 'mobile_money' ? 'selected' : '' }}>
                        Mobile Money</option>
                </select>
                @error('type') <p class="text-sm text-red-500 mt-1">{{ $message }}</p> @enderror
            </flux:field>

            <flux:input type="text" name="nom" label="Nom du service (ex: BNI, BOA, MVola)"
            ...
            <flux:input type="text" name="details" label="Détails (IBAN complet ou Numéro de téléphone)"
                value="{{ old('details', $compte->details) }}" required />
            @error('details') <p class="text-sm text-red-500 mt-1">{{ $message }}</p> @enderror

            <div class="mt-4">
                <flux:switch name="actif" label="Compte Actif" :checked="$compte->actif" />
